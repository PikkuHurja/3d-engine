...
struct cube_map{
    ...
    inline static const glm::quat rotations[6] = {
            //maby should use RH?
        glm::quat_cast(glm::lookAt(glm::vec3(0), directions[0], up_vectors[0])),  // +X
        glm::quat_cast(glm::lookAt(glm::vec3(0), directions[1], up_vectors[1])),  // -X
        glm::quat_cast(glm::lookAt(glm::vec3(0), directions[2], up_vectors[2])),   // +Y
        glm::quat_cast(glm::lookAt(glm::vec3(0), directions[3], up_vectors[3])),  // -Y
        glm::quat_cast(glm::lookAt(glm::vec3(0), directions[4], up_vectors[4])),  // +Z
        glm::quat_cast(glm::lookAt(glm::vec3(0), directions[5], up_vectors[5]))   // -Z
    };
    ...
    inline static struct ::projection_t projection(){
    ...
    inline static glm::mat4 view(const glm::vec3& position, const uint face_index){
        return glm::lookAt(position, position + directions[face_index], up_vectors[face_index]);
    }
};