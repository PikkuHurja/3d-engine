...
void main(){
    ...
    InstanceIndex = gl_InstanceID; //instance_mapping[gl_InstanceID];
    mat4 model_matrix = model_matrix_array[InstanceIndex*2];
    //mat4 inverse_model_matrix = model_matrix_array[InstanceIndex*2+1];
    ...
    vec3 T = normalize(vec3(model_matrix * vec4(aTangent, 1.0)));
    Normal = normalize(vec3(model_matrix * vec4(aNormal, 1.0)));
